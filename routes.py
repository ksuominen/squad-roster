...
def ownpage():
    ...
    if not user_id:
        return render_template(
            "error.html", message="Sorry, you don't have access to this page."
        )
...
def add_item():
    ...
    if not session.get("is_admin"):
        return render_template(
            "error.html", message="Only admins can create new items."
        )

    if request.method == "POST" and form.validate():
        if item.exists(form.name.data):
            return render_template(
                "error.html", message="An item already exists with that name."
            )
        else:
...
def edit_item(item_id):
    if not session.get("is_admin"):
        return render_template(
            "error.html", message="Sorry, only admins can edit items."
        )
...
def add_skill():
    ...
    if not session.get("is_admin"):
        return render_template(
            "error.html", message="Only admins can create new skills."
        )
    if request.method == "POST" and form.validate():
...
def edit_skill(skill_id):
    if not session.get("is_admin"):
        return render_template(
            "error.html", message="Sorry, only admins can edit skills."
        )

    form = f.CreateSkillForm(request.form)
    if request.method == "POST" and form.validate():
        skill.edit_skill(
            skill_id, form.name.data, form.description.data, form.level.data
        )
...
def add_class():
    ...
    if not session.get("is_admin"):
        return render_template(
            "error.html", message="Only admins can create new classes."
        )
    if request.method == "POST" and form.validate():
        mothershipClasses.add_class(
            form.name.data,
            form.stat_adjustment.data,
            form.trauma_response.data,
            form.class_skills.data,
        )
...
def edit_class(class_id):
    if not session.get("is_admin"):
        return render_template(
            "error.html", message="Sorry, only admins can edit classes."
        )

    form = f.CreateClassForm(request.form)
    if request.method == "POST" and form.validate():
        mothershipClasses.edit_class(
            class_id,
            form.name.data,
            form.stat_adjustment.data,
            form.trauma_response.data,
...
def campaigns():
    ...
    if session.get("user_name"):
        player_campaigns = campaign.get_all_campaigns_with_player(
            session.get("user_id")
        )
...
def add_campaign():
    ...
    if not user_id:
        return render_template(
            "error.html", message="You must be logged in to create a campaign."
        )
    if request.method == "POST" and form.validate():
...
def edit_campaign(campaign_id):
    ...
    if not user_id or not campaign.is_gm(user_id, campaign_id):
        return render_template(
            "error.html", message="Sorry, only gamemaster can edit the campaign."
        )
...
def show_campaign(campaign_id):
    ...
    access_campaign = campaign.has_character_in_campaign(
        user_id, campaign_id
    ) or campaign.is_gm(user_id, campaign_id)

    if not user_id or not access_campaign:
        return render_template(
            "error.html", message="Only gamemaster and players can view this page."
        )
...
def remove_campaign_character(campaign_id, character_id):
    ...
    user_id = session.get("user_id")
    can_remove_character = user_id == character_player_id or campaign.is_gm(
        user_id, campaign_id
    )
    if not can_remove_character:
        return render_template(
            "error.html",
            message="Only character's player or campaign's gamemaster can remove a character from a campaign.",
        )
...
def remove_campaign(campaign_id):
    ...
    if not campaign.is_gm(user_id, campaign_id):
        return render_template(
            "error.html", message="Only the gamemaster can delete a campaign."
        )
...
def show_character(character_id):
    ...
    if (
        request.method == "POST"
        and add_skill_form.add_skill_submit.data
        and add_skill_form.validate()
    ):
        if character_info.player_id != user_id:
            return render_template(
                "error.html", message="Only character's player can add skills to them."
            )
        if character.has_skill(character_info.id, add_skill_form.skill_id.data):
            return render_template(
                "error.html", message="Character already has that skill!"
            )
    ...
    if (
        request.method == "POST"
        and add_item_form.add_item_submit.data
        and add_item_form.validate()
    ):
        if character_info.player_id != user_id:
            return render_template(
                "error.html", message="Only character's player can add items to them."
            )
        character.add_item(
            character_info.id, add_item_form.item_id.data, add_item_form.amount.data
        )
        return redirect(f"/character/{character_info.id}")
...
def add_character():
    ...
    if not user_id:
        return render_template(
            "error.html", message="You must be logged in to create a character."
        )
    if request.method == "POST" and form.validate():
...
def edit_character(character_id):
    character_info = character.get_character_info(character_id)
    user_id = session.get("user_id")
    can_edit = user_id == character_info.player_id or campaign.is_gm(
        user_id, character_info.campaign_id
    )
    ...
    if request.method == "POST" and form.validate():
        if form.current_hp.data > form.max_hp.data:
            return render_template(
                "error.html", message="Current hp can't be more than max hp."
            )
        if form.current_stress.data < form.min_stress.data:
            return render_template(
                "error.html", message="Current stress can't be less than min stress."
            )
        ...
        character.edit_character(
            ...
            form.body.data,
            form.max_hp.data,
            form.current_hp.data,
            form.min_stress.data,
            form.current_stress.data,
            form.description.data,
        )
...
def delete_character_skill(character_id, skill_id):
    ...
    if session.get("user_id") != character_player_id:
        return render_template(
            "error.html", message="Only character's player can delete skills from them."
        )
...
def delete_character_item(character_id, item_id):
    ...
    if session.get("user_id") != character_player_id:
        return render_template(
            "error.html", message="Only character's player can use items from them."
        )
...
def remove_character(character_id):
    ...
    if session.get("user_id") != character_player_id:
        return render_template(
            "error.html", message="Only character's player delete them."
        )
