...
class CreateCharacterForm(Form):
    ...
    strength = IntegerField("Strength (1-100):", validators=[validators.NumberRange(min=1, max=100, message="Strength needs to be between 1 and 100")])
    speed = IntegerField("Speed (1-100):", validators=[validators.NumberRange(min=1, max=100, message="Speed needs to be between 1 and 100")])
    intellect = IntegerField("Intellect (1-100):", validators=[validators.NumberRange(min=1, max=100, message="Intellect needs to be between 1 and 100")])
    combat = IntegerField("Combat (1-100):", validators=[validators.NumberRange(min=1, max=100, message="Combat needs to be between 1 and 100")])
    sanity = IntegerField("Sanity (1-100):", validators=[validators.NumberRange(min=1, max=100, message="Sanity needs to be between 1 and 100")])
    fear = IntegerField("Fear (1-100):", validators=[validators.NumberRange(min=1, max=100, message="Fear needs to be between 1 and 100")])
    body = IntegerField("Body (1-100):", validators=[validators.NumberRange(min=1, max=100, message="Body needs to be between 1 and 100")])
    max_hp = IntegerField("Max hp (20-100):", validators=[validators.NumberRange(min=20, max=100, message="Max hp needs to be between 20 and 100")])
    min_stress = IntegerField("Min stress (2-20):", validators=[validators.NumberRange(min=2, max=20, message="Min stress needs to be between 2 and 20")])
    description = StringField("Description")
...
class EditCharacterForm(Form):
    ...
    min_stress = IntegerField("Min stress (2-20):", validators=[validators.NumberRange(min=2, max=20, message="Min stress needs to be between 2 and 20")])
    current_stress = IntegerField("Current stress:", validators=[validators.NumberRange(min=2, max=20, message="Current stress needs to be between min stress and 20")])
    description = StringField("Description")
