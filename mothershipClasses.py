...
def edit_class(id, name, stat_adjustment, trauma_response, class_skills):
    sql = text("UPDATE class SET name=:name, stat_adjustment=:stat_adjustment, trauma_response=:trauma_response, class_skills=:class_skills WHERE id=:id")
    db.session.execute(sql, {"id":id, "name":name, "stat_adjustment":stat_adjustment, "trauma_response":trauma_response, "class_skills":class_skills})
    db.session.commit()
